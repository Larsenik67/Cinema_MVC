<?php
require_once "model/manager/AbstractManager.php";
...
class FilmManager extends AbstractManager
{
     ...
     */
    public function insertFilm($name, $price, $descr)
    {
        $this->executeQuery(
            "INSERT INTO film (name, price, description) VALUES (:n, :p, :d)",
            [
                ":n" => $name,
                ":p" => $price,
                ":d" => $descr
     ...
     */
    public function updateFilm($id, $name, $price, $descr, $image = null)
    {
    ...
    {
        $stmt = $this->executeQuery(
            "SELECT * FROM film WHERE price BETWEEN :min AND :max",
            [
                ":min" => $min,
                ":max" => $max
            ]
